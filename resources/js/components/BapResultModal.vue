...
import { generateConclusionInspection } from '@/lib/pdf/generateConclusionInspection';
...
const requiredFields = {
    ...
    kesimpulanHasil: 'Kesimpulan hasil pemeriksaan',
};
...
const handleSubmit = async () => {
    try {
        ...
        const pdf = generateConclusionInspection(props.report, form);
        const pdfBlob = pdf.output('blob');
        const pdfFile = new File(
            [pdfBlob],
            'laporan-bap-hasil.pdf',
            {
...
<template>
    <BaseModal
        :open="open"
        title="Surat BAP Penyampaian Hasil"
...
"
    ...
    >
        ...
        <section>
            <FormSectionTitle title="2. KESIMPULAN HASIL PEMERIKSAAN" />
            <TextareaField
                name="kesimpulanHasil"
                v-model="form.kesimpulanHasil"
                label="Tuliskan kesimpulan hasil pemeriksaan"
                placeholder="Tuliskan kesimpulan hasil yang akan disampaikan kepada pelapor.."
                rows="5"
                :error="stepErrors.kesimpulanHasil"
                required
            />
        </section>
        <HandlingTeamSection
