...
export const generateConclusionInspection = (
    report: Report | any,
    form: any,
) => {
    const pdf = new jsPDF({
    ...
    const drawHeader = () => {
        ...
        pdf.setFontSize(11);
        pdf.setFont('helvetica', 'bold');
        pdf.text('LAPORAN HASIL KESIMPULAN PEMERIKSAAN', pageWidth / 2, y, {
            align: 'center',
        });
    ...
    // 3. Render Kesimpulan Hasil Pemeriksaan
    drawSectionTitle('II. KESIMPULAN HASIL PEMERIKSAAN');
    drawMultiLineText(form.kesimpulanHasil);
    y += 6;
