...
export const generateVindication = (report: Report | any, form: any) => {
    const pdf = new jsPDF({
        orientation: 'p',
        unit: 'mm',
        format: 'a4',
    ...
    const drawHeader = () => {
        ...
        pdf.setFontSize(11);
        pdf.setFont('helvetica', 'bold');
        pdf.text('DOKUMEN PEMULIHAN NAMA BAIK', pageWidth / 2, y, {
            align: 'center',
        });
    ...
    drawSectionTitle('II. PEMULIHAN NAMA BAIK YANG AKAN DILAKUKAN');
    drawMultiLineText(form.pemulihanAkan);
    y += 6;

    drawSectionTitle('III. HASIL PEMULIHAN NAMA BAIK');
    drawMultiLineText(form.hasilPemulihan);
    y += 6;

    drawSectionTitle('IV. TIM PENANGANAN KASUS');
    drawDataRow(
